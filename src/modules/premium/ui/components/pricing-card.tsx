...
const pricingCardVariants = cva("rounded-lg p-4 py-6 w-[300px]", {
  variants: {
    variant: {
...
const pricingCardBadgeVariants = cva(
  "text-black dark:text-white text-xs font-normal p-1",
  {
    variants: {
      variant: {
        ...
        >
          {features.map((feature,index)=>(
            <li key={index} className="flex items-center gap-x-2.5">
              <CircleCheckIcon className={cn(pricingIconVariants({ variant }))}/>
              {feature}
            </li>
          ))}
        </ul>
